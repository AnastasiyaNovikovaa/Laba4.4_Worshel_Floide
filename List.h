...
template <typename Data>
ostream& operator << (std::ostream& out, List<Data>& list) {
	if (list.is_empty()) {
		out << "\nThe list is empty\n";
	}
	else {
...
template <typename Data>
ostream& operator << (ostream& out, List<Data>* list) {
	if (list->is_empty()) {
		out << "\nThe list is empty\n";
	}
	else {
...
public:
	...
	bool is_in(Data data);
	int find(Data data);      //returns the index of the element or -1 if the element's not found
...
template <typename Data>
bool List<Data>::is_in(Data data) {
	Node* node = head;
	while (node) {
		if (node->data == data)
			return true;
